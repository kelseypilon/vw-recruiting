...
async function sendEmail(
  ...
  await resend.emails.send(payload as Parameters<typeof resend.emails.send>[0]);
}
...
async function processNoScorecardTrigger(
  ...
  for (const interview of interviews) {
    ...
    if (hoursSince >= team.threshold_scorecard_hours) {
      const alreadySent = await hasBeenNotified(supabase, "no_scorecard", interview.id, null, 1);
    ...
    if (hoursSince >= team.threshold_escalation_hours && escalationContact) {
      const alreadySent = await hasBeenNotified(supabase, "no_scorecard", interview.id, null, 2);
      if (!alreadySent) {
        await sendEmail(
          resend,
    ...
    if (hoursSince >= 72 && escalationContact) {
      const alreadySent = await hasBeenNotified(supabase, "no_scorecard", interview.id, null, 3);
      if (!alreadySent) {
        await sendEmail(
          resend,
          escalationContact.email,
        ...
        // Update escalation level on interview
...
async function processHoldFollowUpTrigger(
  ...
  for (const interview of holdInterviews) {
    ...
    if (daysSinceFollowUp >= 0) {
      const alreadySent = await hasBeenNotified(supabase, "hold_followup", interview.id, null, 1);
    ...
    if (daysSinceFollowUp >= 3 && holder) {
      const alreadySent = await hasBeenNotified(supabase, "hold_followup", interview.id, null, 2);
      if (!alreadySent) {
        await sendEmail(
          resend,
          holder.email,
    ...
    if (daysSinceFollowUp >= 8 && escalationContact) {
      const alreadySent = await hasBeenNotified(supabase, "hold_followup", interview.id, null, 3);
      if (!alreadySent) {
        await sendEmail(
          resend,
          escalationContact.email,
...
async function processStuckCandidateTrigger(
  ...
  for (const candidate of candidates) {
    ...
    if (daysInStage >= team.threshold_stuck_days) {
      const alreadySent = await hasBeenNotified(supabase, "stuck_candidate", null, candidate.id, 1);
      if (!alreadySent) {
    ...
    if (daysInStage >= team.threshold_stuck_days + 3 && escalationContact) {
      const alreadySent = await hasBeenNotified(supabase, "stuck_candidate", null, candidate.id, 2);
      if (!alreadySent) {
        await sendEmail(
          resend,
          escalationContact.email,
    ...
    if (daysInStage >= team.threshold_stuck_days + 7 && escalationContact) {
      const alreadySent = await hasBeenNotified(supabase, "stuck_candidate", null, candidate.id, 3);
      if (!alreadySent) {
        await sendEmail(
          resend,
          escalationContact.email,
